...
const ru = {
  ...
  strictEmptyTitle: "Полностью зелёного маршрута не нашлось",
  strictEmptyBody: "В строгом режиме мы не подставляем обычный быстрый маршрут вместо него. Ниже — что нашёл поиск: сколько на каждом варианте зелени и где он теряет время.",
  ...
  tollShort: "платка",
  notStrictGreen: "не весь зелёный",
  notStrictGreenHint: "Есть участки, которые не подтверждены зелёными, поэтому в строгом режиме этот вариант не предлагается",
  tollFreeShort: "без платки",
  externalMayDiffer: "Внешний навигатор может проложить между точками иначе — в том числе по платной дороге.",
  unpaved: "Есть грунтовые участки",
  mapRegion: "Карта маршрута",
  mapFallbackTitle: "Карта OpenStreetMap",
  mapFallbackBody: "Проверьте ключ JavaScript API и HTTP Referer: слой пробок провайдера пока недоступен.",
  mapFallbackDemo: "В демо нет ключа провайдера: карта из OpenStreetMap, слой пробок недоступен.",
...
const en: typeof ru = {
  ...
  strictEmptyTitle: "No fully green route was found",
  strictEmptyBody: "Strict mode does not substitute an ordinary fast route for one. Below is what the search did find: how much of each is green and where it loses time.",
  ...
  tollShort: "toll",
  notStrictGreen: "not all green",
  notStrictGreenHint: "Some segments are not confirmed green, so strict mode does not offer this route",
  tollFreeShort: "toll-free",
  externalMayDiffer: "The external app may route between the points differently, including over a toll road.",
  unpaved: "Includes unpaved sections",
  mapRegion: "Route map",
  mapFallbackTitle: "OpenStreetMap",
  mapFallbackBody: "Check the JavaScript API key and HTTP Referer: the provider traffic layer is unavailable.",
  mapFallbackDemo: "The demo ships no provider key: the map comes from OpenStreetMap and there is no traffic layer.",
