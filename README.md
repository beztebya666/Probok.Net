...
запрос на тот же маршрут с `avoid tolls` и кладёт результат в общий список. Дальше они
конкурируют по доле зелени, и при равном результате бесплатный поднимается выше: платить за
то же самое незачем. Отключается флагом `ENABLE_TOLL_FREE_PROBE=false`.
