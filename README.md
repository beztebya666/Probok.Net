...
Отдельный случай — платные дороги. Если найденный маршрут идёт по платке, поиск тратит один
запрос на тот же маршрут с `avoid tolls` и кладёт результат в общий список. Дальше он
конкурирует по доле зелени наравне со всеми — порядок в списке определяет только зелень.
Платка отмечена значком `₽` на карточке, чтобы выбор был осознанным: когда бесплатный вариант
оказывается такой же зелёный, это видно сразу. Отключается флагом `ENABLE_TOLL_FREE_PROBE=false`.
...
> Всё выше снято с боевого стека: карта и слой пробок — 2ГИС, маршруты — реальный ответ
> провайдера, сохранённый и обезличенный. Так выглядит приложение с ключами.
...
экспорт приложения в демо-режиме.

<table>
<tr>
<td width="66%"><img src="docs/media/demo-desktop-light.webp" alt="Демо: тот же анализ на карте OpenStreetMap"></td>
<td width="34%"><img src="docs/media/demo-mobile-dark.webp" alt="Демо на телефоне"></td>
</tr>
</table>

Разница с боевым стеком одна: карта из OpenStreetMap вместо 2ГИС и без слоя пробок — ключа
провайдера в публичной статике нет и быть не должно. Всё остальное настоящее: те же
замороженные ответы провайдера, те же цвета участков, тот же поиск по фикстурам. Поиск отвечает из фикстур прямо в браузере, поэтому сайт не
